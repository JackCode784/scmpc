...
interruptBool = true;
scnConstr = false; % include scenarios constraints
scnCost = false; % include scenarios in cost
...
for k=1:nSim
    ...
    % Zonotope update
    sigmas(k) = sigma;
    ZNonReds{k+1} = boundStripZonotopeInt(ZReds{k}, ySim(k), yInit, uInit, noiseAmp, nb, nk, 'nr');
    ZReds{k+1} = reduceOrder(ZNonReds{k+1}, 'gs');

    % New reduction method
    minRad = min(radi(1), radi(k));
    vn = vecnorm(ZReds{k+1}.G);
    ZReds{k+1}.G(:,vn > minRad) = ZReds{k+1}.G(:,vn > minRad) ./ vn(vn > minRad) * minRad;

    P = polytope(ZReds{k+1}) & polytope(Z0);

    yInit = [ySim(k), yInit(1:end-1)];

    % Optimization
    % [thetaScenarios, thetaNominal] = scenary(ZReds{k+1}, nScen);
    thetaNominal = ZReds{k+1}.c;
    thetaScenarios = P.randPoint(nScen);

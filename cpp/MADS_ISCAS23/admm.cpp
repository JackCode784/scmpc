...
void admm(fxd v1[nDim_CTRL], fxd v2[nCon + (N+1)*nX_CTRL_sc], fxd z[nDim_CTRL])
{
	#pragma HLS allocation operation instances=mul limit=DIM_TO_REPLACE
	...
	fxd v2_reg[nCon + (N+1)*nX_CTRL_sc];
	...
	fxd_tmp_m lambda_tmp[nCon + (N+1)*nX_CTRL_sc];
	#pragma HLS ARRAY_PARTITION variable=lambda_tmp dim=1 complete

	fxd lambda_new[nCon + (N+1)*nX_CTRL_sc];
	...
	if (init == true)
	{
			for (int i = 0; i < nCon + (N+1)*nX_CTRL_sc; i++)
			{
				#pragma HLS UNROLL
				y[i] = 0;
				lambda[i] = 0;
	...
	for (int k = 0; k < max_iter; k++)
	{
		...
		for (int j = 0; j < nCon + (N+1)*nX_CTRL_sc; j++)
		{
			//#pragma HLS allocation operation instances=mul limit=DIM_TO_REPLACE
			#pragma HLS UNROLL //small off

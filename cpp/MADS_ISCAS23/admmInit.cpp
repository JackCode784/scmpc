...
void admmInit(fxd x[nX_CTRL], fxd ref[nX_CTRL], fxd v1[nDim_CTRL], fxd v2[nCon + (N+1)*nX_CTRL_sc])
{
...
ref*/
  ...
  for (int i = 0; i < (N+1)*nX_CTRL_sc; i++)
  {
    #pragma HLS UNROLL
    b[i] = 0;
  }
...
ref*/
  ...
  for (int i = nCon; i < nCon + (N+1)*nX_CTRL_sc; i++)
  {
    #pragma HLS UNROLL
    v2_reg[i] = b[i-nCon];
  }

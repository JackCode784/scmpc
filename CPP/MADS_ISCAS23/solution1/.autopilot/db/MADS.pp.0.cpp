...
# 2 "MPCwithMADS_fxd/MADS.cpp" 2

void MADS(data_u optimum[2], data_x x[2], data_x par[2], data_x ref[1]) {


 data_meshExp frameIdx[2];


 data_meshExp meshIdx[2];


 data_u pollMatrix[2][2*2];


 data_cost initialCost[2];

 VITIS_LOOP_17_1: for (int i = 0; i < 2; i++) {
  frameIdx[i] = D[i];
 }
 ...
 VITIS_LOOP_25_2: for (int i = 0; i < 7; i++) {


  VITIS_LOOP_28_3: for (int i = 0; i < 2; i++) {
   if (frameIdx[i] < -12) {
    break;

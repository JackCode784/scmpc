# 1 "MPCwithMADS_fxd/MPCwithMADS.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 395 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "MPCwithMADS_fxd/MPCwithMADS.cpp" 2
# 1 "MPCwithMADS_fxd/myLib.h" 1




# 1 "MPCwithMADS_fxd/boostParams.h" 1
...
# 10 "MPCwithMADS_fxd/boostParams.h" 2
...
# 55 "MPCwithMADS_fxd/boostParams.h"
static const ap_ufixed<1, 0, AP_TRN, AP_WRAP> expC = 0.5;


static const ap_ufixed<3, 3, AP_TRN, AP_WRAP> Q[2][2] = { { 5, 0 }, { 0, 1 } };
...
# 75 "MPCwithMADS_fxd/boostParams.h"
static const data_meshExp D[2] = { -8, -8 };
# 84 "MPCwithMADS_fxd/boostParams.h"
static const data_u startingPoint[2] = { 0.5, 0.5 };
...
# 134 "MPCwithMADS_fxd/boostParams.h"
void costFunction(data_cost cost[2], data_u point[2], data_x x[2], data_x par[2], data_x ref[1]);
# 6 "MPCwithMADS_fxd/myLib.h" 2

__attribute__((sdx_kernel("MPCwithMADS", 0))) void MPCwithMADS(data_in x[2], data_in par[2], data_in ref[1], data_out uOpt[1]);
void shiftOpt(data_u optimum[2]);
void MADS(data_u optimum[2], data_x x[2], data_x par[2], data_x ref[1]);
void generatePollMatrix(data_u currentPoint[2], data_meshExp frameSize[2], data_meshExp meshSize[2], data_u pollMatrix[2][2 * 2]);
void generatePollDirections(data_rand randomVector[2], data_meshExp frameSize[2], data_meshExp meshSize[2], data_dir directions[2][2*2]);
void generateHouseholderMatrix(data_rand v[2], data_rand H[2][2]);

void pseudoRand(data_rand randomVector[2]);
void progressiveBarrierPolling(data_cost cost[2], data_u currentPoint[2], data_u pollMatrix[2][2*2], data_meshExp frameSize[2], data_x x[2], data_x par[2], data_x ref[1]);
# 2 "MPCwithMADS_fxd/MPCwithMADS.cpp" 2

__attribute__((sdx_kernel("MPCwithMADS", 0))) void MPCwithMADS(data_in x[2], data_in par[2], data_in ref[1], data_out uOpt[1]) {_ssdm_SpecArrayDimSize(x, 2);_ssdm_SpecArrayDimSize(par, 2);_ssdm_SpecArrayDimSize(ref, 1);_ssdm_SpecArrayDimSize(uOpt, 1);
#pragma HLS TOP name=MPCwithMADS
# 3 "MPCwithMADS_fxd/MPCwithMADS.cpp"
...
#pragma HLS ARRAY_PARTITION variable=x dim=1 complete
 ...
 static data_u optimum[2];


 VITIS_LOOP_21_1: for (int i = 0; i < 2; i++) {
  x_int[i] = x[i];
 }


 VITIS_LOOP_26_2: for (int i = 0; i < 2; i++) {
  par_int[i] = par[i];
 }


 VITIS_LOOP_31_3: for (int i = 0; i < 1; i++) {
  ref_int[i] = ref[i];
 }


 if (firstCall) {
  VITIS_LOOP_37_4: for (int i = 0; i < 2; i++) {
 ...
 VITIS_LOOP_47_5: for (int i = 0; i < 1; i++) {
  uOpt[i] = optimum[i];
 }

# 1 "MPCwithMADS_fxd/progressiveBarrierPolling.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 395 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "MPCwithMADS_fxd/progressiveBarrierPolling.cpp" 2
...
# 2 "MPCwithMADS_fxd/progressiveBarrierPolling.cpp" 2
...
#pragma HLS ARRAY_PARTITION variable=costTest dim=1 complete
 ...
 VITIS_LOOP_17_1: for (int i = 0; i < 2*2; i++) {
#pragma HLS PIPELINE



 VITIS_LOOP_22_2: for (int j = 0; j < 2; j++) {
#pragma HLS UNROLL
 testPoint[j] = pollMatrix[j][i];
  ...
  if (costTest[1] > cost[1]) {
   ...
   VITIS_LOOP_42_3: for (int j = 0; j < 2; j++) {
#pragma HLS UNROLL
 ...
 if (success == true) {
  VITIS_LOOP_55_4: for (int i = 0; i < 2; i++) {
#pragma HLS UNROLL
 frameSize[i] = frameSize[i] + 1;
  }
  success = false;
 } else {
  VITIS_LOOP_61_5: for (int i = 0; i < 2; i++) {
#pragma HLS UNROLL
 frameSize[i] = frameSize[i] - 1;
  }
 }
